...
We read your "Practical Challenges for Researchers in Data Sharing" White Paper and found the analysis to be quite convincing and precient.  Perhaps I could a couple further observations which may have something to do with slow adoption of rigorous data-curation protocols.  One, there's limited support from science applications -- you have software which can save data in domain-specific formats, like ITK for bioimaging, OME-TIFF for microscopy, Protein Data Bank for molecular visualization, FCS for Flow Cytometry, etc., but these applications are not (as of know) compiled with libraries to bundle these raw data files with metadata and manuscripts to create a sharable data set or "Research Object".  Two, there's also a variety of competing standards for describing/encoding data sets (DCC, MIBBI, schema.org/Dataset, Research Objects, etc.).  So, that may cause confusion for authors uncertain which specification to use for their publishing environment.  (Our proposal is to merge these various standards into one Object-Oriented system).  

One final point: there has not been a lot of momentum behind formulating general-purose data-representation standards for encoding scientific data.  HDF5 and ARFF are fairly broad-based and cross-disciplinary, but each is not completely expressive (they are targeted at a restricted set of data structures).  We believe that a more comprehensive representation format can be derived from Hypergraph Database Engines, so we're trying to standardize a hypergraph query format wherein we hope to get contributions from the important examples of this database architecture (e.g. HyperGraphDB, Graken.ai, AtomSpace, perhaps lMNTal and WhiteDB).  We believe a cross-engine hypergraph query format could serve double-duty as a highly expressive data-representation language (at least modulo F-omaga limitations, if I may use type theory terms), so we'd really like to involve HyperGraphDB and others alongside publishers as part of an informal standardization pilot.  By extension, there was some innovative work on Hypergraph Categories in the linguistics/semantics context recently done during a year-long seminar at Cambridge University, which merged hypergraphs with Conceptual Space Theory, and that latter theory has also been developed as a data-representation and "philosophy of science" paradigm by certain researchers, so that's another modeling space that could be factored in.         
