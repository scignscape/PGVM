...
\ifnum\value{page}=21   %\pgfmathresult>0\relax
\put(41,132){
...
\geometry{left=.8in,top=.74in,right=.8in,bottom=1.4in} %margins
...
\newcommand{\HTXN}{\resizebox{!}{8pt}{\ATexttclr{HTXN}}}
...
\newcommand{\Grakenai}{\resizebox{!}{8pt}{\AcronymText{Graken.ai}}}
...
\vspace{5pt}
\textit{\large{Improving Data Sets' Machine Readability \\
...
(Minimum Information for Biological and Biomedical Investigations).  The purpose of \SDRF{} is to merge these different data-set formats into a unified overarching standard (which can be adapted to different publishing houses and organization pipelines).  This would be of benefit to publishers because it simplifies their technologies for interacting with data sets.  And in fact, \SDRF{} would allow publishers to work with one single data-sharing protocol even though publishers' portals are connected with multiple data-hosting platforms.}
...
IJST (pending successful peer review) the disparate open-access 
...
\q{Supplemental Archive;} and (7) annotating the data sets to support 
...
\item{} \textbf{Representations of Laboratory Protocols 
and/or Computational Workflows} \hspace{14pt}  Formal 
representations of research protocols and workflows 
facilitate reuse of the data set and/or replication of 
the research work, as will be shown in the next section.

\end{enumerate}
}

\section{Documenting Research Protocols: Making Beneficial Modifications to BioCoder}

\p{It is understood that a systematic delineation of research 
methods/protocols 
serves a number of different purposes.  In some cases, documenting 
laboratory protocols promotes replication of the 
methodology used --- in the physical 
environment of a laboratory, reproducing a lab protocol 
is dependent on precise configurations of measurements and 
equipment that often cannot be gleaned from the text of 
a research paper alone.  This is the motivation for 
protocol-description formats such as those gathered 
under \MIBBI{} or generated by \BioCoder{}.  For example, 
when seeking to reproduce 
the methods discussed in a published article, lab technicians 
often consult \MIBBI{} 
files for technical information about the experimental 
setup.  Such files supplement what appears in the printed article, 
which often does not provide sufficient detail about 
experimental parameters.}

\p{\BioCoder{} (a programming framework to standardize 
and automate biology protocols so that they may be replicated in other 
laboratories) offers a variation on 
laboratory-based protocol-descriptions such as \MIBBI{}.  This is 
because although \BioCoder{} procedures generate 
protocol-description files, 
the actual definition of each protocol is constructed via procedural 
(specifically, \Cpp{}) computer code.  That is to say, 
\BioCoder{}-defined protocols can be examined on two levels: 
through descriptions (in formats such as \HTML{}) which are 
generated after \BioCoder{} is executed, or through the 
actual \Cpp{} code where the steps and requirements of 
the protocol are set forth in granular detail.  Employing 
\Cpp{} as a modeling language allows protocol-descriptions 
to be constructed in an expressive manner, utilizing 
programming formations (such as branching statements, 
objects, template/generic programming, and so forth) 
to represent protocol details.  Modelers, as such, are 
afforded the full expressiveness of a procedural 
programming language to work with (rather than the 
more restricted expressiveness of a markup language).  
At the same time, \BioCoder{}'s libraries are equipped 
to generate (for example) \HTML{} files by executing 
the \Cpp{} code wherein a protocol is defined; consequently, 
the protocol is also expressed in human-readable form, 
so that consulting the underlying \Cpp{} programming 
is not a prerequisite for studying the protocol 
being described.}

\p{Our \SDRF{} code libraries include a modified version of 
\BioCoder{} intended to expand the range of environments 
where \BioCoder{} may be utilized.  The key additions include:

\begin{enumerate}
\item{}  Using a more modern 
\Cpp{} programming style --- for example, with \Qt{} and \STL{} 
(Standard Template Library) containers/algorithms instead of 
\C{} arrays.  This approach makes it easier to adopt \BioCoder{} in 
conjunction with other \Cpp{} libraries for \PDF{}, \LaTeX{}, 
graph visualization, application networking, and so forth.
 
\item{}  Allowing templates and abstract base classes to be 
established as an interface for \BioCoder{}-style workflow classes.  
That is, apart from concrete protocols, there can also be \q{protocol models} 
which reflect different investigative modalities --- e.g., 
protocols for Machine Learning need a different vocabulary than protocols for 
lab experiments.  Consequently, existing \BioCoder{} global functions could, 
in such contexts, be mapped to class methods in this more general framework. 
For example, \textbf{start\_protocol(...)} would become
\textbf{dspin.start\_protocol(...)}.\footnote{Here \sDSPIN{} refers 
to an \sSDRF{} model for image analysis that is under-development.}
 
\item{}  Generating protocol-description files in multiple formats 
(not only \HTML{}), and integrating protocol models with publishing software.  
For instance, \BioCoder{}'s description generators may be extended to output 
\LaTeX{}, \TAGML{}, or publisher-specific \XML{} dialects.  
Moreover, each step in a described 
protocol could be assigned a unique identifier, which in turn is used to
create a unique \LaTeX{} label, permitting locations within the text 
of the publications associated with the research work to be annotated with
links to the protocol description.
 
\item{}  Extending \BioCoder{}'s graph functionality, using a 
\Qt{}-compatible library to visualize the runtime graph, with a 
convenient interface for manipulating the graph structure via modern \Cpp{} 
code.  Insofar as protocol models can be 
tailored to disparate scientific fields,
different protocols would also call for varying semantics for
...
\item{} Aligning protocol models with
standardized workflow vocabularies and \q{minimum information}
checklists, such as \MIBBI{}.  For runtime graphs, such ontologies could
...
protocol model might be decorated with annotations asserting that a
particular function notates a research step which has a particular
label in an \MIBBI{} (or similar) recommendation.  One may, for
instance, maintain a dictionary of links between \Cpp{} function/method
pointers and label names formalized in an \MIBBI{}-style controlled vocabulary.
\end{enumerate}
}

\subsection{Research Protocols and Computational Workflows: Integrating 
Code with Supplemental Archives}

\p{Technologies such as \BioCoder{} and \MIBBI{} are primarily 
applied to the construction of 
human-readable summaries of research protocols.  
A related use-case involves explicitly notating 
workflows for \q{\textit{in silico}} experiments or any 
other research which depends on digital analysis or processing.  
For example, investigations which depend on image-analysis 
may implement analytic routines wherein sample images are 
subject to a sequence of transformations, with a 
modified image produced after each step constituting 
input for the next step.  With proper imaging software, 
researchers could reproduce such analyses explicitly --- whether to 
verify the accuracy of the published analysis or to 
better understand the published research by visualizing 
its intermediary stages.  In these situations, 
workflow descriptions are not only human-readable guidelines 
for re-enacting a certain investigative modality; they 
can be formal computational artifacts which orchestrate 
the execution of implemented procedures so as to allow the original 
computations to be repeated in different environments.}

\p{Researchers will sometimes share their analytic code 
in the same spirit of transparency and reusability as 
that which motivates sharing raw data.  At a minimum, 
such sharing need entail only including source-code files 
within a data set and/or linking data sets to version-controlled 
archives (on GitHub, for instance, or a similar code-hosting site).  
However, computer code can be more tightly integrated with 
their containing Supplemental Archives by including code 
annotations and/or documentation which concretely ties 
programming elements (such as procedures, types, object member 
fields, or individual typed values) to components and 
concepts that can be cited in the data set (and annotated 
in a corresponding publication).  In some cases, such 
annotations may serve to explain why a given algorithmic 
implementation or sequence of operations is chosen, 
yielding an explication of digital protocols similar 
to those documented (in the lab context) via \BioCoder{} 
or \MIBBI{}.  The \q{\Pandore{}} project, for instance --- 
a suite of image-processing utilities (called \q{operators}) 
that can be combined into compound workflows --- 
introduces an \q{Image Processing Objectives} ontology 
yielding systematically defined lexical notation for 
asserting the purpose of different steps in an 
image-processing pipeline.  Workflows modeled 
via \Pandore{} may be directly executed via imaging 
software, and they may also be studied as human-readable 
elucidations clarifying why the shared image-processing 
analyses are composed as they are.}

\p{\lSDRF{} encourages authors to utilize systematic annotation 
systems spread across text, code, and data as much as is feasibly possible.  
Because the details of annotation semantics depends on the article's 
applicable scientific field, it is difficult to 
present a general analysis of how such annotations might be 
constructed.\footnote{Although some disciplines have existing 
standard vocabularies that can be drawn upon for source-code 
annotations, including the \sPandore{} ontology, and the \sMIFlowCyt{} 
specification within \sMIBBI{}, to mention examples cited here.}  
(LTS is also working on a more cross-disciplinary, but 
experimental, mechanism for 
unifying code and text annotations as part of \THQL{}, 
discussed in Appendix II below.)  
}

%\p{Use protocols as executable workflows for "in silico" modalities.
...
\item{}  A figure shows one rendering of a \ThreeD{} manifold displaying the distribution of a high-dimensional data set under dimensional reduction.  Via a context menu, this graphic can be viewed in 
...
\item{}  An archaeology paper shows images from a site which has been investigated with advanced imaging/laser equipment, e.g. point-cloud scanners or Matterport 360-degree cameras.  Via context menus or links embedded in figure captions, the associated data set may be 
rendered in \ThreeD{} graphics software such as \MeshLab{}, or in Panoramic Photography viewers such as Panini.
...
\item{}  A paper on stroke rehabilitation might use \DICOM{} to record both image data (e.g. \MRI{}s) and audio data (e.g. documenting aphasia).  These \DICOM{} files need to be opened in the proper software -- perhaps \medInria{} for \TwoD{} images, \ITK{} Snap for \ThreeD{} images, and Audacity for audio.  When the publication text describes the diagnostic significance of particular \DICOM{}-encoded findings, then (perhaps via \DICOM{} Structured Reporting), these text segments should be annotated to identify the software category appropriate for the specific \DICOM{} files used.
\end{enumerate} 
}
...
implemented in mainstream programming languages (arrays, lists, tuples, 
