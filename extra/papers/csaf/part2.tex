...
\IeXML{}\footnote{See \bhref{https://www.semanticscholar.org/paper/IeXML\%3A-towards-an-annotation-framework-for-semantic-Rebholz-Schuhmann-Kirsch/1d72a56b6576117c62f388a5f2193965e4c7e293}.} and \JATS{} (Journal Article Tag Suite).  Despite (or perhaps because of) this diversity of 
representational possibilities, guidelines for constructing 
and annotating scientific data remain open-ended: researchers 
...
manipulate data and metadata structured in 
...
This motivates the idea of a \q{Scientific Data Repository Model} 
(\SDRM{}), concretized within code implementing 
...
the Scientific Data Repository Model]{Interoperating with Science Portals via 
the \protect\q{Scientific Data Repository Model}}
...
those just enumerated and potentially 
other groupings.\footnote{For instance, data in some fields --- such 
...
potential example of \sSDRM{} units would be 
...
publications (and users of corresponding data sets) 
...
from downloaded data sets, or some combination thereof. 
(It is better to make at least part of the \SDRM{} data available 
without having to download data sets as a whole, because 
a useful overview of the data set may contribute to  
whether users choose to download it \textit{ab initio}.)}
...
\begin{scriptsize}
\begin{center}
\begin{tabular}{|p{.12\textwidth}|p{.23\textwidth}|p{.25\textwidth}|p{.2\textwidth}|}
...
--- as is the case with Dataset Applications
(discussed in the next section) --- then such 
\GUI{}s may include dialog boxes 
...
Ideally, dataset code may be distributed both 
via plugins to domain-specific software 
popular for a given research field, 
and also via standalone Dataset Applications 
accessible to researchers coming from other disciplines.}
...
instance, \dsC{} projects are structured to use \Qt{}'s \q{\textbf{qmake}} 
build system as the primary tool for compiling 
data-set code.  The common \dsC{} code therefore 
includes \textbf{qmake} project files which support 
...
\LaTeX{}-to-\PDF{} converters) generate files 
in a custom \CsAF{} hypergraph-serialization format 
(\HGXF{}, for \q{Hypergraph Exchange Format,})  
which store information about textual and 
...
(in the biomedical context) patient narratives.  To establish 
annotation targets for cross-references 
between publications and data-set code, \dsC{} 
employs a code-annotation system based 
on hypergraphs, along with (on the manuscript side) the Annotation 
Exchange Format (\AXF{}) cited above (see 
Page~\lpageref{ph:axf}).}
