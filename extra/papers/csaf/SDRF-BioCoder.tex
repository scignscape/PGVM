...
\p{It is understood that a systematic deliniation of research 
methods/protocols 
...
and automate biology protocols can be replicated in other 
...
\textbf{dspin.start\_protocol(...)}.\footnote{Here \DSPIN{} refers 
to an \SDRF{} model for image analysis that is under-development.}
...
For instance, \BioCoder{}'s description generators can (depending on the 
\BioCoder{} template selected) be extended to output 
...
convenient interface for manipulating the graph structure with modern \Cpp{} 
...
\item{} Establishing a framework for aligning protocol models with
...
label in an \MIBBI{} (or similar) recommendation.  One might, for
...
Code with Supplemental Archive}
...
annotations, including the \Pandore{} ontology, and the \MIFlowCyt{} 
specification within \MIBBI{}, to mention examples cited here.}  
