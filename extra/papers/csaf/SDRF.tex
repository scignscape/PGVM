...
\newcommand{\SDRF}{\resizebox{!}{7.5pt}{\ATexttclr{SDR%
\hspace{-2pt}{\fontfamily{qcr}\fontseries{b}\selectfont{}\large\textit{f}}%
}}}


\newcommand{\lSDRF}{\resizebox{!}{8pt}{\ATexttclr{S}}\resizebox{!}{7.5pt}{\ATexttclr{DR%
\hspace{-2pt}{\fontfamily{qcr}\fontseries{b}\selectfont{}\large\textit{f}}%
}}}
...
The authors also use an additional source of gait data derived from a more recent (2019) study whose data is availablee only upon request (see \bhref{https://www.nature.com/articles/s41598-019-53656-7#MOESM1}).

\item{}  The speech data is primary drawn from a data set hosted by the University of California Irvine Machine Learning Repository (\bhref{https://archive.ics.uci.edu/ml/datasets/Parkinsons}).  The central information is a \CSV{} file, where each line represents a single voice recording from one of 23 Parkinson's patients or 8 healthy controls (each research subject made multiple recordings).  The individual lines in the \CSV{} file present a quantitative model of subjects' speech via a 
...
or request made of corresponding authors of referenced papers introducing the relevant data 
...
image-feature extraction already performed when the data set was 
first published.  However, the republished unified data set 
(to appear in conjunction with the IJST article) may 
include code to allow researchers to reproduce the original 
analytic workflow if desired; for instance, we can enable the 
...
(see Figure~\ref{fig:s2} here).  In so doing, this properly 
...
(reported in papers cited as data sources for the IJST article) is provided 
...
such information however is not directly included within the PhysioNet data set; it 
...
as \SDRF{} can fill the gap, make scientific data more \q{\FAIR{}} (Findable, Accessible, 
Interoperable, Reusable).  Upon publication of the peer-reviewed article  
...
paragraphs within the Parkinson's article specifically discuss given 
data sets that the authors analyzed, the same data which the \SDRF{} 
archive woyld encode via \Cpp{} classes.  This would enable those paragraphs 
which discuss the data set to be cleanly linked to the 
...
they should be able to automatically launch 
...
of readers having to waste time with manually 
finding, downloading, merging/extracting, 
and then opening data files, these steps would be 
...
the case of the Parkinson's data, however, there is no single 
