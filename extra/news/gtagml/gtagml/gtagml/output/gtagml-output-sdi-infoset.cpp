...
void GTagML_Output_SDI_Infoset::mark_sentence(QTextStream& qts, caon_ptr<tNode> node, u4 enter, u4 leave)//, u4 pleave)
{
...
//   enter, leave, notes);
 ...
 while(i <= leave)
 {
  ...
  if(!sdi_document_->scan_for_sentence_start(bl, i, leave, ss))
    break;
  u4 se = 0;
  if(!sdi_document_->scan_for_sentence_end(bl, i, leave, se))
    break;
