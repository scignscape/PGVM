...
GTagML_Output_Infoset::GTagML_Output_Infoset(GTagML_Document& document) //, HTXN_Infoset_8b* infoset)
...
void GTagML_Output_Infoset::init_callbacks()
{
 #include "gtagml-output-callbacks-common.h"

 #include "gtagml-output-infoset.callbacks.h"
}
...
void _insert_end(QTextStream& qts, u8* u, QVector<caon_ptr<GTagML_Node>>* vec, u4 index,
  ...
  QMap<u4, caon_ptr<GTagML_Node>>& marked_sentence_starts,
...
void _insert_start(QTextStream& qts, QVector<caon_ptr<GTagML_Node>>* vec, u4 index, QString& pre_result, caon_ptr<GTagML_Node> n, u4& canceled_sdi_sentence_start_index)
{
...
void _insert_end(QTextStream& qts, u8* u, u4 index, u4 new_index, 
  QString& post_result, QMap<u4, caon_ptr<GTagML_Node>>& marked_sentence_starts,
  u4& held_sdi_sentence_end_index,  
  u4& canceled_sdi_sentence_start_index)
{
...
void _insert_end(QTextStream& qts, 
  ...
  QString& post_result, 
  QMap<u4, caon_ptr<GTagML_Node>>& marked_sentence_starts, caon_ptr<GTagML_Node> n,
  u4& held_sdi_sentence_end_index,
  u4& canceled_sdi_sentence_start_index, 
  caon_ptr<GTagML_Node>* held_sdi_sentence_end_node)
{
...
u8 GTagML_Output_Infoset::check_sdi_latex_insert(GH_Block_Base* bl, u4 index, QString& pre_result, QString& post_result)
{
 u8 count = 0;
...
QVector<caon_ptr<GTagML_Node>> GTagML_Output_Infoset::get_sdi_latex_insert_nodes(GH_Block_Base* bl, u4 index, QString& pre_result, QString& post_result)
{
...
void GTagML_Output_Infoset::write_infoset_output(QString& output)
{
 QTextStream qts(&output);
 generate(qts);
}

void GTagML_Output_Infoset::generate(QTextStream& qts)
{
 GTagML_Output_Event_Generator events(*this, *this);
 events.generate(qts);
}

void GTagML_Output_Infoset::generate_root(const GTagML_Output_Bundle& b, caon_ptr<GTagML_Root> nr)
{
}

void GTagML_Output_Infoset::export_infoset(QString path)
{
 if(path.startsWith(".."))
 {
  path.remove(0, 1);
...
void GTagML_Output_Infoset::export_infoset_review(QString path)
{
 if(path.startsWith(".."))
 {
  path.remove(0, 1);
...
caon_ptr<GTagML_Tag_Command_Callback> GTagML_Output_Infoset::check_command_callback(caon_ptr<GTagML_Tag_Command> ntc)
{
...
//caon_ptr<GTagML_Tag_Command_Callback> GTagML_Output_Infoset::check_callback
...
void GTagML_Output_Infoset::check_post_callback
...
void GTagML_Output_Infoset::generate_tag_command_auto_leave(const GTagML_Output_Bundle& b, caon_ptr<GTagML_Tag_Command> ntc)
{ 
 CHECK_SUPPRESS_NODE
//? b.qts << "/>";
}

void GTagML_Output_Infoset::generate_tag_command_argument(const GTagML_Output_Bundle& b,
...
void GTagML_Output_Infoset::check_generate_tag_command_argument(const GTagML_Output_Bundle& b,
...
void GTagML_Output_Infoset::generate_sdi_mark(caon_ptr<GTagML_Tag_Command> ntc,
...
void GTagML_Output_Infoset::generate_tag_command_entry(const GTagML_Output_Bundle& b, GTagML_HTXN_Node& nhn)
{ 
// CHECK_SUPPRESS_NODE
// HTXN_Node_Detail* nd = nhn.get_node_detail(htxn_document_);
// Glyph_Layer_8b* bl = nd->get_layer();
...
void GTagML_Output_Infoset::generate_tag_command_entry(const GTagML_Output_Bundle& b, caon_ptr<GTagML_Tag_Command> ntc)
{ 
...
void GTagML_Output_Infoset::check_sentence_boundaries(QTextStream& qts, caon_ptr<tNode> node)
{
 CAON_PTR_DEBUG(tNode ,node)
 if(caon_ptr<GTagML_Tag_Command> ntc = node->GTagML_tag_command())
 {
  CAON_PTR_DEBUG(GTagML_Tag_Command ,ntc)
  if(GH_Prenode* ghp = 0) // = ntc->prenode())
  {
   u4 enter;// = ghp->get_ref_enter();
   if(enter > 1)
   {
    ...
    if(leave >= enter)
    {
     ...
     check_sentence_boundaries(qts, node, //*ghp,
                               enter, leave);
    } 
   }
  }
...
void GTagML_Output_Infoset::mark_sentence(QTextStream& qts, caon_ptr<tNode> node, u4 enter, u4 leave)//, u4 pleave)
{
...
void GTagML_Output_Infoset::check_sentence_boundaries(QTextStream& qts,
  caon_ptr<tNode> node, //GH_Prenode& ghp,
                                                      u4 enter, u4 leave)
{ 
...
//   enter, leave, notes);
 ...
 {
  u4 ss = 0;
//  if(!htxn_document_->scan_for_sentence_start(bl, i, leave, ss))
//    break;
  u4 se = 0;
...
void GTagML_Output_Infoset::generate_tag_command_leave(const GTagML_Output_Bundle& b, GTagML_HTXN_Node& nhn)
{ 
// HTXN_Node_Detail* nd = nhn.get_node_detail(htxn_document_);
...
void GTagML_Output_Infoset::generate_tag_command_leave(const GTagML_Output_Bundle& b,
...
void GTagML_Output_Infoset::generate_tag_body_leave(const GTagML_Output_Bundle& b, GTagML_HTXN_Node& nhn)
{
 CHECK_SUPPRESS_NODE
// HTXN_Node_Detail* nd = nhn.get_node_detail(htxn_document_);
...
void GTagML_Output_Infoset::generate_tag_body_leave(const GTagML_Output_Bundle& b, caon_ptr<GTagML_Tag_Command> ntc)
{ 
 CHECK_SUPPRESS_NODE
 CAON_PTR_DEBUG(GTagML_Tag_Command ,ntc)
...
//void GTagML_Output_Infoset::generate_tile_via_htxn(const GTagML_Output_Bundle& b, GTagML_HTXN_Node& nhn)
//{
////? CHECK_SUPPRESS_NODE
